...
current_aliases.each do |line|
  ...
  # ]
  #
  # the result for each audio channel:
  # [
  #     [0] ":0",
  #     [1] "ngrand"
  # ]
...
while continue
  ...
  break if new_alias.nil?

  # select outputs that currently are not assigned an alias
  choices = prompt.multi_select("select outputs to assign alias", outputs, per_page: 16, max: 2)
  ...
  choices.each do |port|
    ...
    # remove port from selection list
    outputs.delete_if { |x| x == port }
  end

end

unless monitor_ports.empty?
