...
case "$(echo $input | cut -d " " -f 1)" in
	...
	dic)
		...
		if ! [ -z $query ]
		then
			exec $BROWSER "https://google.com/search?q=Define+$query" &> /dev/null &
		fi
		;;
	*)
	  # Search the web
	  ...
	  query=$(echo "$input" | cut -c 1- | xargs -0)
		if ! [[ -z $query ]]
		then
			exec $BROWSER "https://google.com/search?q=site+stackoverflow.com+$query" &> /dev/null &
			sleep 0.25
			exec $BROWSER "https://google.com/search?q=site+%22unix.stackexchange.com%22+$query" &> /dev/null &
			sleep 0.25
			exec $BROWSER "https://google.com/search?q=site+gist.github.com+$query" &> /dev/null &
			sleep 0.5
			exec $BROWSER "https://google.com/search?q=$query" &> /dev/null &
		fi
	  ;;
